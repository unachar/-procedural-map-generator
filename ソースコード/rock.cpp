...
void Rock::Initialize()
{
	...
	m_ModelRenderer->Load("asset\\model\\rock\\rock.obj");

	Renderer::CreateVertexShader(
		&m_VertexShader,
		&m_VertexLayout,
		"shader\\rockInstanceVS.cso"
	);
	...
	Light.Ambient = XMFLOAT4(1.3f, 1.3f, 1.3f, 1.0f);
	Light.Direction = XMFLOAT4(0.2f, -1.0f, 0.0f, 0.0f);

	// インスタンスバッファ作成
	D3D11_BUFFER_DESC bd = {};
	bd.Usage = D3D11_USAGE_DYNAMIC;
	bd.ByteWidth = sizeof(RockInstanceData) * MAX_INSTANCES;
...
void Rock::Draw()
{
	...
	// 頂点バッファ + インスタンスバッファ設定
	UINT strides[2] = { sizeof(VERTEX_3D), sizeof(RockInstanceData) };
	UINT offsets[2] = { 0, 0 };
...
void Rock::ClearInstances()
{
	m_InstanceData.clear();
	m_InstanceCount = 0;
}

void Rock::AddInstance(const XMMATRIX& world)
{
...
void Rock::UpdateInstanceBuffer()
{
	if (m_InstanceCount == 0) return;

	D3D11_MAPPED_SUBRESOURCE msr;
	Renderer::GetDeviceContext()->Map(m_InstanceBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &msr);
	memcpy(msr.pData, m_InstanceData.data(), sizeof(RockInstanceData) * m_InstanceCount);
